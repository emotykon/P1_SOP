...
void sync_restore(const char *curr_src, const char *curr_dst, const char *root_src, const char *root_dst) {
    ...
    while ((dir = readdir(d)) != NULL) {
        ...
        if (S_ISDIR(st.st_mode)) {
            ...
            // Uwaga: root_src to tutaj cel (oryginalny folder), root_dst to backup
            // Musimy odwrócić logikę przeliczania linków
            copy_symlink(d_path, s_path, root_dst, root_src);
        } else {
            copy_file(d_path, s_path, 1); // Update only
        }
    }
    closedir(d);

    // 2. Usuń z Source (src) to czego nie ma w Backup (dst)
    d = opendir(curr_src);
    if (!d) return;
    while ((dir = readdir(d)) != NULL) {
        if (strcmp(dir->d_name, ".") == 0 || strcmp(dir->d_name, "..") == 0) continue;
...
void handle_restore(const char *dest_path) {
    ...
    if (!realpath(dest_path, dest_real)) {
        printf("Błąd: Niepoprawna ścieżka kopii.\n");
        return;
    }
    ...
    if (!src_real) {
        ...
        printf("Przywracanie możliwe tylko dla aktywnych sesji (by znać źródło).\n");
        return;
    }

    printf("Przywracanie...\nZ:  %s\nDo: %s\n", dest_real, src_real);
    // root_src tutaj to folder ORYGINALNY, root_dst to BACKUP (do funkcji resolve_symlink)
    sync_restore(src_real, dest_real, src_real, dest_real);
    printf("Przywracanie zakończone.\n");
}